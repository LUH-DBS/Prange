...
class Tably:
    ...
    def run(self):
        ...
        if self.outfile or self.separate_outfiles is None:
            ...
            if self.outfile:
                try:
                    ...
                    logger.error(
                        '{} is not a valid/known path. Could not save there.'.format(self.outfile))
            else:
                print(final_content)
        ...
        if self.separate_outfiles is not None:
            ...
            if len(outs) == 0:
                outs = [os.path.splitext(
                    file)[0]+'.tex' for file in self.files]
            elif os.path.isdir(outs[0]):
                outs = [os.path.join(outs[0], os.path.splitext(
                    os.path.basename(file))[0])+'.tex' for file in self.files]
            elif len(outs) != len(self.files):
                logger.warn(
                    'WARNING: Number of .csv files and number of output files do not match!')
    ...
    def create_table(self, file):
        ...
        try:
            ...
            return ''
        if not rows:
            logger.warn("No table created from the {} file. Check if the file is empty "
    ...
    def save_single_table(self, file, out):
        ...
        if table:
            ...
            try:
                ...
                logger.error(
                    '{} is not a valid/known path. Could not save there.'.format(out))
...
def format_alignment(align, length):
    ...
    columns.
    """
    if any(ch not in 'lcrS' for ch in align):
        align = 'c'
...
def save_content(content, outfile, replace):
    ...
    if replace:
        ...
        logger.debug('The content is written to' + outfile)
    else:
        with open(outfile, 'a') as out:
            out.writelines(content)
        logger.debug('The content is appended to' + outfile)


def arg_parser():
    """Parses command line arguments and provides --help"""
    parser = argparse.ArgumentParser(
        description="Creates LaTeX tables from .csv files")
