...
It should be emphasized that the column header is not being extracted as a feature. While this could lead to a better performance in some cases, it is challenging to encode the various headings in a way that is understandable for the machine learning model. % TODO: is this good?
...
For this thesis, the algorithms and experiments where implemented using \texttt{Python 3.10.2}. To interact with the data, the python library \texttt{pandas} was used as its dataframe structure has useful additional functions and has a good compatibility to the library \texttt{Auto-Sklearn}, which was used to train the machine learning model.
