\begin{table}[ht]
    \caption[Efficiency experiment on tables with \SI{30}{\percent} unique columns saved as parquet files, where only the necessary rows and columns are loaded]{The result of the efficiency experiment on a generated table with \SI{30}{\percent} unique columns in a parquet file format. The experiment was conducted on a model with an input size of 10 rows on tables generated with 10 columns. During the experiment, only the necessary rows and columns were loaded.}
    \small
